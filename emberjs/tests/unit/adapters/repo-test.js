...
test('it should return data when get success', function(assert) {
  ...
  this.server.respondWith('GET', starredURL,
    [200, {'Content-Type': 'application/json'},
      starredData]);
  adapter.query(null, null, 'hawkup').then((response) => {
    assert.ok(response);
  });
  this.server.respond();
});
...
test('it should return error data when get not found', function(assert) {
  let adapter = this.subject();
  this.server.respondWith('GET', notFoundURL,
    [404, {'Content-Type': 'application/json'},
      notFound]);
  adapter.query(null, null, 'hawkupppp').catch((error) => {
    assert.equal(error.message, 'Ajax operation failed');
  });
  this.server.respond();
});
